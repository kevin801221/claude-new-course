...
| `class.table.frozen` | s02 | `{version, nc, names, naming_rationale}` | M2 |
...
| `charts.series` | s08 | `{curves\|pr\|confusion\|per_class}` | M4 |
...
| `stop` | s11 | `{reason, best_run, best_pt}` | M7 |
...
4. 每 15 秒發一行 `: ping` 註解撐反向代理與背景分頁節流。
...
```
- `class_table_version` 在 M1 一律 `null`（class 真相表 M2 才凍）。前端看到 `null` 要顯示「未分類」而不是空字串。
...
```json
{"run_id":"r7","status":"cancelled","stop_seq":813}
```
- M1 語意：停掉 s01–s03 的產生器、寫一筆 `stop {reason:"user_cancel"}`、run 標 `cancelled`。**M1 不轉發 `train/cancel`、不關 autonomy**（那兩條在 M3/M7 補）。
...
`source:"roboflow"` / `"local"` → `501`（M8 / 未排程）。
...
run 的 `status` 不在 `queued`/`running` → `409 RUN_NOT_LIVE`（同 §8.5）。
...
| `501` | `NOT_IMPLEMENTED` | §9 的佔位端點；或 M1 傳了 `mode:"manual"/"autonomous"`、`source:"roboflow"/"local"`、`label.auto mode:"cluster"` |
