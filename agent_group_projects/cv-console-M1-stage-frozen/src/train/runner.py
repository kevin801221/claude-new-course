...
# ---------- cancel = SIGTERM(pid)，收乾才算停 ----------

def _signal(pid: int, sig: int) -> None:
    """整個 process group 一起送（dataloader worker 之類的孫子也要死），拿不到 group 就退回單一 pid。"""
    try:
...
def cancel(train_id: str) -> dict[str, Any]:
    ...
    return {"train_id": train_id, "status": "cancelled", "stop_seq": event["seq"], "pid": pid}
