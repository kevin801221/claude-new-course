...
def build(*, result: dict[str, Any], records: Sequence[dict[str, Any]], names: Sequence[str],
          ...
          noise_floor: float | None = None, op_conf: float | None = None,
          ceiling: dict[str, Any] | None = None) -> dict[str, Any]:
    """`GET /eval/{run_id}/charts` 的回應本體（四張圖 + 標註天花板一次到位）。

    `ceiling` 是 `src.eval.ceiling` 那份摘要，**照原樣**送（不在這裡挑欄位）：前端那張
    「標註天花板」直方圖要靠 `degenerate` / `credible` / `note` 把「合成退化 1.0」與
    「真人工 GT」畫成兩種可信度 —— 少送一個旗標，兩者就會長得一樣可信。
    """
    ...
    return {
        ...
        # ci_dropped / ci_note 一起送：CI 是 None 的時候前端要說得出「為什麼沒有誤差線」，
        # 不然那條線消失得跟「畫壞了」一模一樣（eval-notes §8 的加欄慣例，不改既有欄名/語意）。
        "per_class": [{k: p[k] for k in ("cls", "name", "ap50", "ap", "recall", "precision",
                                         "recall_at_conf", "best_f1_conf", "support",
                                         "ci_lo", "ci_hi", "ci_dropped", "ci_note") if k in p}
...
def _selfcheck() -> None:  # pragma: no cover - 手動跑的自檢
    ...
    out = build(result=metrics.with_ci(res, n_boot=50), records=records, names=names,
                ...
                ceiling={"source": "synthetic", "degenerate": True, "credible": False,
                         "iou_median": 1.0, "iou_hist": [0] * 9 + [155], "threshold": 0.6})
    assert set(out) >= {"curves", "pr", "confusion", "per_class", "speed", "noise_floor", "ci",
                        "ceiling"}
    assert out["per_class"][0]["ci_lo"] is not None
    # 天花板照原樣送出去：`degenerate` 掉了的話，前端畫出來的退化 1.0 會跟真人工 GT 一樣可信
    assert out["ceiling"]["degenerate"] is True and out["ceiling"]["credible"] is False, out["ceiling"]
    assert build(result=metrics.with_ci(res, n_boot=50), records=records, names=names,
                 results_csv=None, speed=None)["ceiling"] is None      # 沒量到就是 None，不要假的
