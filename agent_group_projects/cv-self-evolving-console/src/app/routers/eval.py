...
torch / ultralytics 一行都不准出現在本檔 —— 推論走子行程（`app/procs.py`）起 `src/eval/infer.py`。
"""
...
def _infer(*, weights: Path, split: str, out_path: Path, imgsz: int, device: str) -> dict[str, Any]:
    ...
    # 走 procs.run（登記進表 + 自成一組）：這支被 `asyncio.to_thread` 包著，而 to_thread
    # 不可取消 —— 按了 Stop 只有訊號打得到它（契約 §8.4）。
    proc = procs.run(cmd, cwd=str(PROJECT_ROOT), capture_output=True, text=True,
...
def evaluate(body: EvalBody) -> JSONResponse:
    ...
    # 下限不是 1：n_boot 太小時百分位撐不住 —— 實測 n_boot=1 回 {lo:hi:同一個點, width:0.0}，
    # `significant()` 的第二道（|Δ| < CI 半寬）恆為 false → 護欄 3 的 CI 那一半被一個參數關掉，
    # 而前端照樣把寬度 0 當「95% bootstrap CI」畫成一個點。
    if not metrics.MIN_BOOT <= body.n_boot <= 20000:
        raise _err(400, "BAD_RANGE",
                   f"n_boot 必須在 {metrics.MIN_BOOT}..20000，收到 {body.n_boot}"
                   "（低於下限的重抽數撐不住 95% 百分位，等於把護欄 3 的 CI 判準關掉）")
    ...
    # 鎖 3b：版本字串沒變、但**表被換掉了**。`class_table_version` 是人填的字串，
    # 重新凍結一次它不會動（實測 `_Context/class_table.json` 被後面一個 run 覆蓋成 nc=3，
    # version 還是 v1，而 02-dataset 的標籤用到 cls 5）→ 舊版在這裡不是拒答，是
    # `series.confusion()` 的 `IndexError` 500，錯誤訊息指不到真因。
    fit = metrics.class_table_fits_labels()
    if not fit["ok"]:
        raise _err(409, "INCOMPARABLE_CLASS_TABLE",
                   f"class 真相表現在是 {fit['nc']} 類 {fit['names']}，但 02-dataset 的標籤用到 "
                   f"cls {fit['max_cls']} —— 表被換過而 class_table_version 沒有跟著動"
                   f"（還是 {current_ctv}）。這一份 eval 算出來的 per-class 會對錯表，拒答。"
                   "修法：把 _Context/class_table.json 換回這份 02-dataset 凍結當時那一版"
                   "（每個 ds 自己那份在 01-raw-data/datasets/<ds>/class_table.json），"
                   "或重新 freeze 一次資料集")
    ...
    bus.append_event(body.run_id, STAGE, "eval.noise_floor", ACTOR,
                     ...
                     text=f"noise floor 2σ = {nf['noise_floor']:.4f}"
                          ...
                          f"anchor gate 另有自己的帶 2σ = "
                          f"{nf['anchor_noise_floor'] if nf.get('anchor_noise_floor') is not None else '未量'}"
                          "（不同 metric 不共用雜訊帶）")
    ...
    if body.split == "valid":
        a_pred = _infer(weights=weights, split="anchor", out_path=eval_dir / "anchor.json",
                        imgsz=imgsz, device=device)
        anchor_recall = metrics.anchor_recall_of(a_pred["records"], names)

    # 標註天花板（s03 的 label.anchor_iou + 真人工 GT 的實測）—— 這一輪的數字有沒有上限，
    # 以及那個上限是「合成退化 1.0」還是「真人工 GT」。合成那條不重量（退化是結構事實）。
    ceil = ceil_mod.for_run(body.run_id, ds_id=state.get("ds_id"), n_boot=body.n_boot)
    ceiling_source = (ceil or {}).get("source")
    ...
    for s in reversed(_all_summaries()):
        ...
        if body.compare_to and s.get("run_id") != body.compare_to:
            continue
        # 舊的 summary 沒有 `ceiling_source` 欄位（本輪之前寫的）→ 回它自己的
        # `label.anchor_iou` 事件去問，不要用預設值猜（猜錯就是拿兩把尺算 Δ）。
        prev_ceiling = s.get("ceiling_source") or ceil_mod.source_of_run(s.get("run_id") or "")
        bad_ceiling = not ceil_mod.comparable(ceiling_source, prev_ceiling)
        if (s.get("class_table_version") != ctv or s.get("ds_fingerprint") != fingerprint
            ...
            incomparable.append({"run_id": s.get("run_id"),
                                 ...
                                 "ceiling_source": prev_ceiling,
                                 "why": "class_table_version 不同" if s.get("class_table_version") != ctv
                                        else "資料集指紋不同（同一個 ds_version 底下資料已被換掉）"
                                        if s.get("ds_fingerprint") != fingerprint
                                        else f"天花板來源不同（{prev_ceiling} vs {ceiling_source}）"
                                             "：合成退化值與真人工 GT 是兩把尺，之間沒有可比的 Δ"})
            continue
        prev = s
        break
    ...
    check_list = [
        ...
        # anchor gate 吃 **anchor recall 自己的 2σ**，不是 val mAP50-95 的（差 2.1 倍，
        # 拿錯會把純 seed 抖動判成「立刻 abandoned」）。沒量到就讓 anchor_drift 拒答。
        ck.anchor_drift(anchor_recall, (prev or {}).get("anchor_recall"), delta,
                        nf.get("anchor_noise_floor")),
        ck.overfit_gap(curves, nf["noise_floor"]),
        # 第五項：標註天花板。這一格決定「問題在標註還是在模型」——
        # 低於門檻 → fatal → verdict invalid（模型再好也頂不破標註）；
        # 合成退化值 → PASS 但 credible=False（沒量到天花板 ≠ 天花板很高）。
        ck.label_ceiling(ceil),
    ]
    ...
    summary = {
        ...
        "anchor_noise_floor": nf.get("anchor_noise_floor"), "anchor_sigma": nf.get("anchor_sigma"),
        ...
        # 天花板是「跟誰可比」的第三把鎖（前兩把是 class_table_version 與 ds_fingerprint）：
        # 合成退化 1.0 與真人工 GT 之間的 Δ 沒有意義，寫進 summary 才守得住下一輪。
        "ceiling_source": ceiling_source, "ceiling": ceil,
    ...
    return JSONResponse(status_code=202, content={
        ...
        "verdict": vd["verdict"], "anchor_ok": anchor_ok, "ceiling": ceil,
        "checks": [{"name": c["name"], "pass": c["pass"], "detail": c["detail"]}
                   for c in check_list],
...
def noise_floor(ds_version: str, measure: bool = False, run_id: str | None = None,
    ...
    # 「量完」要含 anchor recall 的帶：只有 val 那條的舊快取在這裡直接回的話，
    # measure_noise_floor 裡的補量路徑永遠走不到，anchor gate 就永遠拒答。
    if metrics.noise_floor_complete(cached) and not force:
...
# ---------- GET /eval/ceiling（標註天花板，**不必先訓練**） ----------

@router.get("/eval/ceiling")
def ceiling(run_id: str | None = None, ds: str | None = None, scope: str = "pool",
            measure: bool = True, force: bool = False,
            n_boot: int = metrics.N_BOOT) -> dict[str, Any]:
    """auto-bbox 對人工 GT 的 IoU 分佈 —— 整條流水線的天花板。

    兩種問法：
      `?run_id=rNN`  → 這個 run 的天花板（讀它 s03 的 `label.anchor_iou`；真人工 GT 那條
                       另外實測 pool 並附 95% CI）。**s03 跑完就問得到，不必等訓練**，
                       DESIGN 的 `label_ceiling` 放棄條件要的就是這個時機。
      `?ds=dsNN`     → 這份資料集本身的天花板（還沒有 run 也能問）。

    回應多帶一個 `gate`：`{abandon, reason:"label_ceiling", credible, detail}` ——
    `abandon=true` 的處方是**停在 s03**，不是照樣訓練然後在 s08 說「數字不好」。

    🔒 `scope` 只接受 `anchor` / `pool`。sealed-test 的 80 張人工框是考卷答案，
    任何 scope 都拿不到它（`ceiling.measure()` 直接 ValueError，不靠本檔自律）。
    """
    if not metrics.MIN_BOOT <= n_boot <= 20000:
        raise _err(400, "BAD_RANGE", f"n_boot 必須在 {metrics.MIN_BOOT}..20000，收到 {n_boot}")
    if run_id:
        state = _state(run_id)
        payload = ceil_mod.for_run(run_id, ds_id=ds or state.get("ds_id"),
                                   measure_real=measure, n_boot=n_boot)
        if payload is None:
            raise _err(409, "CEILING_UNAVAILABLE",
                       f"run {run_id} 還沒有 label.anchor_iou 事件（s03 還沒跑完）——"
                       "天花板要等 auto-label 量過才有")
    elif ds:
        try:
            payload = ceil_mod.measure(ds_id=ds, scope=scope, n_boot=n_boot, force=force)
        except ValueError as exc:
            raise _err(409, "CEILING_UNAVAILABLE", str(exc)) from exc
    else:
        raise _err(400, "BAD_ENUM", "要帶 run_id 或 ds 其中一個（run_id 問這一輪、ds 問這份資料集）")
    return {"run_id": run_id, "ds": ds, **payload, "gate": ceil_mod.gate(payload)}
...
def verdict(run_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "anchor_noise_floor": summary.get("anchor_noise_floor"),
        "anchor_sigma": summary.get("anchor_sigma"),
        ...
        "ceiling": summary.get("ceiling"), "ceiling_source": summary.get("ceiling_source"),
